...
from state_diff.diffusion_policy.model.common.normalizer import LinearNormalizer
from state_diff.diffusion_policy.policy.base_lowdim_policy import BaseLowdimPolicy
# from state_diff.diffusion_policy.model.diffusion.conditional_unet1d import ConditionalUnet1D
from state_diff.diffusion_policy.model.diffusion.mask_generator import LowdimMaskGenerator
from state_diff.diffusion_policy.model.common.nn import mean_flat
from state_diff.diffusion_policy.model.common.resample import create_named_schedule_sampler, LossAwareSampler
from state_diff.diffusion_policy.policy.base_lowdim_policy import apply_conditioning_init
...
class EquivariantStateDiffWithInverseModel(BaseLowdimPolicy):
    def __init__(self, 
        ...
        if 'deno_save_every' in kwargs:
            self.deno_save_every = kwargs['deno_save_every']
            del kwargs['deno_save_every']
        else:
            self.deno_save_every = None
        ...
        if not self.skip_inv_model:
            ...
            self.inv_model = self.create_equiv_model()

    def create_equiv_model(self):

        
        N = 8 # same as equi diffpo
        
        # Define the cyclic group without a base space
        group = gspaces.no_base_space(CyclicGroup(N))
        self.group = group

        
        in_type = enn.FieldType(group, [group.regular_repr] * (self.inv_model_input_dim // N))
        self.in_type = in_type
        
        # Define activation function
        if self.inv_act == 'relu':
            activation_class = enn.ReLU
        else:
            raise KeyError
        
        
        # Build the equivariant MLP
        hidden_type1 = enn.FieldType(group, [group.regular_repr] * (self.inv_hidden_dim // N))
        layer1 = enn.Linear(in_type, hidden_type1)
        
        hidden_type2 = enn.FieldType(group, [group.regular_repr] * (self.inv_hidden_dim // N))
        layer2 = enn.Linear(hidden_type1, hidden_type2)
        
        # Output layer - we convert to trivial representation for standard output
        # Trivial representation ensures the output is invariant to the group action
        out_type = enn.FieldType(group, [group.trivial_repr] * self.action_dim)
        layer3 = enn.Linear(hidden_type2, out_type)
        
        # Create the sequential model
        model = nn.Sequential(
            layer1,
            activation_class(hidden_type1),
            layer2,
            activation_class(hidden_type2),
            layer3
        )
        return model
    ...
    def conditional_sample(self, 
        ...
        if self.deno_save_every:
            intermediate_num = len(scheduler.timesteps) // self.deno_save_every
            deno_trajectories_shape = (condition_data.shape[0], intermediate_num, *list(condition_data.shape[1:]))
            deno_trajectories = torch.empty(*deno_trajectories_shape, dtype=condition_data.dtype, device=condition_data.device)
        else:
            deno_trajectories = None
        save_deno_idx = 0
        ...
        for idx, t in enumerate(scheduler.timesteps):
            ...
            if self.deno_save_every and (idx + 1) % self.deno_save_every == 0:
                deno_trajectories[:, save_deno_idx] = trajectory.clone()
                save_deno_idx += 1
        ...
        if not self.skip_inv_model:
            ...
            obs_comb = GeometricTensor(obs_comb, type=self.in_type)
            action = self.inv_model(obs_comb)
            # convert back to tensor
            action = action.tensor

            action = action.reshape(B, T - self.n_his_inv - self.n_fut_inv + 1, self.action_dim)
        else:
            action = trajectory[:, self.n_obs_steps:, -self.action_dim:]
        traj_info = {'deno_traj': deno_trajectories,
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], action_slice=None) -> Dict[str, torch.Tensor]:
        ...
        state_traj = self.normalizer['obs'].unnormalize(traj_info['state_traj'] )   
        deno_trajectories = traj_info['deno_traj']

        if deno_trajectories is not None:
            deno_trajectories = deno_trajectories[...,:Da]
            deno_trajectories = self.normalizer['action'].unnormalize(deno_trajectories, action_slice)
        ...
        if self.pred_action_steps_only:
            ...
            #     state_traj = state_traj[:, start:end]
            if deno_trajectories is not None:
        ...
        result = {
            ...
            'state_traj': state_traj.reshape(B, T, -1, 2) if self.vis_state_traj else None,
        }
        ...
        result['deno_trajectories'] = deno_trajectories
    ...
    def compute_loss(self, batch, action_slice=None):
        ...
        if not self.skip_inv_model:
            ...
            #     pred_a_t = self.inv_model(GeometricTensor(x_comb_t, type=self.in_type))
