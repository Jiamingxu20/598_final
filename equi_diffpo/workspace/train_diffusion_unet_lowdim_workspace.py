...
from equi_diffpo.policy.diffusion_traj_unet_lowdim_policy import DiffusionTrajUnetLowdimPolicy
from equi_diffpo.policy.diffusion_equi_traj_unet_lowdim_policy import EquivariantStateDiffWithInverseModel

from state_diff.diffusion_policy.common.pytorch_util import dict_apply, optimizer_to
from state_diff.diffusion_policy.workspace.base_workspace import BaseWorkspace
from state_diff.diffusion_policy.dataset.base_dataset import BaseLowdimDataset
# from state_diff.diffusion_policy.dataset.robomimic_replay_lowdim_dataset import RobomimicReplayLowdimDataset
from state_diff.diffusion_policy.env_runner.base_lowdim_runner import BaseLowdimRunner
from state_diff.diffusion_policy.common.checkpoint_util import TopKCheckpointManager
from state_diff.diffusion_policy.common.json_logger import JsonLogger
from state_diff.diffusion_policy.model.common.lr_scheduler import get_scheduler
...
class TrainingTimer:
    def __init__(self, total_epochs):
        """Initialize the timer with total number of epochs."""
        self.start_time = time.time()
        self.total_epochs = total_epochs
        self.epoch = 0
        
    def update(self, epoch):
        """Update timer with current epoch."""
        self.epoch = epoch
        ...
        if epoch > 0:  # Avoid division by zero
            time_per_epoch = time_spent / epoch
            time_remaining = time_per_epoch * (self.total_epochs - epoch)
        else:
            time_remaining = 0
            
        # Convert to hours, minutes, seconds
        spent_hrs, remainder = divmod(time_spent, 3600)
        spent_mins, spent_secs = divmod(remainder, 60)
        
        remaining_hrs, remainder = divmod(time_remaining, 3600)
        remaining_mins, remaining_secs = divmod(remainder, 60)
        
        # Print progress
        print(f"Epoch {epoch}/{self.total_epochs} ({epoch/self.total_epochs*100:.1f}%)")
        print(f"Time spent: {int(spent_hrs)}h {int(spent_mins)}m {int(spent_secs)}s")
        print(f"Est. time remaining: {int(remaining_hrs)}h {int(remaining_mins)}m {int(remaining_secs)}s")
...
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if 'traj' in cfg.policy._target_:
            ...
            raise NotImplementedError(f"Policy {cfg.policy._target_} not implemented")
    ...
    def run(self, rank=0, world_size=1):
        ...
        timer = TrainingTimer(total_epochs=cfg.training.num_epochs)

        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                # print time info using timer
                timer.update(local_epoch_idx)
                step_log = dict()
