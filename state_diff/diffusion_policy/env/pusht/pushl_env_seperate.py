...
class PushLEnv(gym.Env):
    ...
    def step(self, action):
        if self.agentmode == 0:
            ...
            action1, action2 = action[:2], action[2:]
        ...
        self.done = (L0_object_on_L0_goal_area > self.success_threshold*L0_goal_geom.area and L1_object_on_L1_goal_area > self.success_threshold*L1_goal_geom.area)
        ...
        total_coverage = (L0_object_on_L0_goal + L1_object_on_L1_goal)
    ...
    def teleop_agent_spacemouse(self, select = 0):
        ...
        TeleopAgent = collections.namedtuple('TeleopAgent', ['act'])
        if select == 0:
            device0 = SpaceMouse(pos_sensitivity=3000.0, rot_sensitivity=3000.0, SpaceMouseNumber=0)
        else:
            device0 = SpaceMouse(pos_sensitivity=3000.0, rot_sensitivity=3000.0, SpaceMouseNumber=1)
    ...
    def _setup(self):
        ...
        self.L1_object = self.add_L1((300, 200), 0)
        # L1_goal_pos = (self.L0_goal_pose[:2].copy())
        # goal_distance = 64
        # L1_goal_pos[0] -= goal_distance
        # L1_goal_pos[1] += goal_distance
        self.L1_goal_pose = np.array([200,300,np.pi*2/4])  # x, y, theta (in radians)
    ...
    def add_circle(self, position, radius, select=0):
        body = pymunk.Body(body_type=pymunk.Body.KINEMATIC)
        body.position = position
        body.friction = 1
        shape = pymunk.Circle(body, radius)
        if select == 0:
            shape.color = pygame.Color(242, 97, 34)
        else:
            shape.color = pygame.Color(18, 4, 76)
