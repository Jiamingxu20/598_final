...
class PushRopeEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.rope_ballspacing = 21

        self.reward = 0

        Dagent = 2
        Dobs = self.rope_length * 2 + Dagent
            ...
            if state is None:
                ...
                for i in range(self.rope_length):
                    if i == 0:
                        ...
                        if (rand_x >= 250):
                            x_increment = -self.rope_ballspacing * np.sin(np.pi/8)
                            y_increment = self.rope_ballspacing * np.cos(np.pi/8)
                        else:
                            x_increment = self.rope_ballspacing * np.sin(np.pi/8)
                            y_increment = self.rope_ballspacing * np.cos(np.pi/8)
                        self.pos_Balls_init = np.append(self.pos_Balls_init, rand_x + i*x_increment)
                        self.pos_Balls_init = np.append(self.pos_Balls_init, 25)
                    else:
                        self.pos_Balls_init = np.append(self.pos_Balls_init, rand_x + i*x_increment)
                        self.pos_Balls_init = np.append(self.pos_Balls_init, 25 + i*y_increment)
            ...
            if state is None:
                ...
                for i in range(self.rope_length):
                    if i == 0:
                        rand_x = rs.randint(25, 475)
                        if (rand_x >= 250):
                            x_increment = -self.rope_ballspacing * np.sin(np.pi/3)
                            y_increment = self.rope_ballspacing * np.sin(np.pi/6)
                        else:
                            x_increment = self.rope_ballspacing * np.sin(np.pi/3)
                            y_increment = self.rope_ballspacing * np.sin(np.pi/6)
                        self.pos_Balls_init = np.append(self.pos_Balls_init, rand_x + i*x_increment)
    ...
    def step(self, action1, action2=None):
        ...
        for i in range(self.rope_length):
            ...
            if curr_y >= 60 and curr_y <= 90 and curr_x >= -70 and curr_x <= 90: # top
                intersection_counter += 1
            elif curr_y >= -90 and curr_y <= -60 and curr_x >= -90 and curr_x <= 70: # bottom
                intersection_counter += 1
            elif curr_y <= -0.857*curr_x + 17.15 and curr_y >= -0.857*curr_x - 17.15 and curr_y <= 60 and curr_y >= -60: # middle
                intersection_counter += 1
        ...
        succ_intersection = int(self.success_threshold*self.rope_length)

        self.done = intersection_counter >= succ_intersection
    
        reward = np.clip(intersection_counter/succ_intersection, 0, 1)
        self.reward = reward
    ...
    def _get_obs(self):
        if self.agentmode == 0:
            ...
            obs = self.pos_Balls
            obs = np.concatenate([obs, np.array([self.agent1.position[0], self.agent1.position[1]])])
            obs = np.concatenate([obs, np.array([self.agent2.position[0], self.agent2.position[1]])])
    ...
    def _set_state(self, state):
        if self.agentmode == 0:
            ...
            for i in range(self.rope_length):
                self.pos_Balls = np.append(self.pos_Balls, state[2+i*2:4+i*2])
            ...
            self.pos_Balls = np.array([])
            for i in range(self.rope_length):
                self.pos_Balls = np.append(self.pos_Balls, state[4+i*2:6+i*2])
            self.agent1.position = pos_agent1
    ...
    def _setup(self):
        ...
        for i in range(self.rope_length-1):
            ...
            # self.add_spring(self.Balls[i], self.Balls[i+1])
    ...
    def add_string(self, body1, body2):
        ...
        # pymunk.constraints.DampedSpring(a: Body, b: Body, anchor_a: Tuple[float, float], anchor_b: Tuple[float, float], rest_length: float, stiffness: float, damping: float)
        # a (Body) – Body a
        # b (Body) – Body b
    ...
    def add_spring(self, body1, body2):
        body_1 = body1
        body_2 = body2
        spring = pymunk.DampedSpring(
            body_1, body_2, (0, 0), (0, 0),
            rest_length=self.rope_ballspacing, stiffness=10, damping=0)
