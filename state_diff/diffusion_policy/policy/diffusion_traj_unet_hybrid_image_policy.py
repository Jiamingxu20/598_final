...
class DiffusionTrajUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
        ...
        # encoder_kwargs['rgb']["core_kwargs"]["flatten"] 
        ...
        # latent_dim = kwargs.get('rgb_latent_dim', 64)
        # obs_net = ObsNets.MIMO_MLP(input_obs_group_shapes=observation_group_shapes, 
        #     output_shapes= OrderedDict(
        #         output_shapes=(latent_dim,),
        #     ),
        #     layer_dims=(300, 400),
        #     encoder_kwargs=encoder_kwargs,
        # )      
        ...
        obs_feature_dim = obs_encoder.output_shape()[0]  # obs_net.output_shape()['output_shapes'][0] 
        ...
        self.inv_guided_encoder = kwargs.get('inv_guided_encoder', False)
        self.n_frame_encoder = kwargs.get('n_frame_encoder', None)
    ...
    def compute_loss(self, batch):
        ...
        if self.training_stage == 0:
            ...
            inv_input_traj = trajectory
            if self.inv_guided_encoder:
                n_his_inv = self.n_his_inv 
                n_fut_inv = self.n_fut_inv
            else:
                if self.n_frame_encoder is None:
                    n_his_inv = self.n_his_inv + self.n_fut_inv
                else:
                    n_his_inv = self.n_frame_encoder
                    total_trained_frames = self.n_his_inv + self.n_fut_inv
                n_fut_inv = 0

            inv_traj_slice_len = T - n_his_inv - n_fut_inv + 1
            # Create a range of indices for slicing
            indices = torch.arange(n_his_inv + n_fut_inv).unsqueeze(1) + torch.arange(inv_traj_slice_len)
            # Use advanced indexing to get the slices in one go
            traj_slices = torch.swapaxes(inv_input_traj[:,indices,:], 1, 2) 
            # Reshape the tensor to the desired shape and concatenate along the last dimension
            if self.n_frame_encoder is not None:
                traj_slices = traj_slices.repeat(1, 1, 1, int(total_trained_frames/self.n_frame_encoder))
 
            x_comb_t = traj_slices.reshape(-1, self.inv_model_input_dim)

            a_t = nactions[:, n_his_inv-1 : T-n_fut_inv, :]
