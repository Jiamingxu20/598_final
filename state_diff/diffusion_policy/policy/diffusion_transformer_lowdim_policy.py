...
class DiffusionTransformerLowdimPolicy(BaseLowdimPolicy):
    ...
    def conditional_sample(self, 
        ...
        return trajectory, deno_trajectories
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # run sampling
        nsample, deno_trajectories = self.conditional_sample(
            cond_data, 
            cond_mask,
            cond=cond,
            **self.kwargs)
        ...
        if deno_trajectories is not None:
            ...
            deno_trajectories = self.normalizer['action'].unnormalize(deno_trajectories)
    ...
    def compute_loss(self, batch):
        ...
        if self.schedule_sampler != None and isinstance(self.schedule_sampler, LossAwareSampler):
            loss = mean_flat(loss)
            if "vb" in terms:
                loss = loss + terms["vb"]*self.kwargs.get('vb_factor', 1)  # TODO
