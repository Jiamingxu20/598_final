...
from diffusion_policy.model.inverse_dynamics.inverse_dyanmics import InverseModel

class DiffusionTrajUnetVideoPolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
    ...
    def create_inv_model(self):
        return InverseModel(inv_hidden_dim=self.inv_hidden_dim, action_dim=self.action_dim, inv_act='relu')
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        To = self.n_obs_steps
        videos = nobs['image'].permute(0, 2, 1, 3, 4)  # video (batch, channels, frames, height, width)
        ...
        sampled_videos = self.model.sample(cond = global_cond)
        sampled_videos = sampled_videos.clone()
        sampled_videos[:, :, :To, ...] = videos 

        nsample = self.inv_model(sampled_videos)
        # # run sampling
    ...
    def compute_loss(self, batch):
        ...
        videos = nobs['image'].permute(0, 2, 1, 3, 4) # video (batch, channels, frames, height, width)
        diffuse_loss = self.model(videos, cond=global_cond)

        pred_a_t = self.inv_model(videos)
        a_t = nactions[:, self.n_his_inv-1 : horizon-self.n_fut_inv, :]
