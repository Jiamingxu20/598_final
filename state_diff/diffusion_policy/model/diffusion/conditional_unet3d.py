...
from diffusion_policy.model.diffusion.text import tokenize, bert_embed, BERT_MODEL_DIM
...
def Upsample(dim):
    return nn.ConvTranspose3d(dim, dim, (1, 4, 1), (1, 2, 1), (0, 1, 0))

def Downsample(dim):
    return nn.Conv3d(dim, dim, (1, 4, 1), (1, 2, 1), (0, 1, 0))
...
class Block(nn.Module):
    def __init__(self, dim, dim_out):
        super().__init__()
        self.proj = nn.Conv3d(dim, dim_out, (1, 3, 1), padding = (0, 1, 0))
...
class Unet3D(nn.Module):
    def __init__(
        ...
        init_padding = (0, init_kernel_size // 2, 0) if channels == 1 else (0, init_kernel_size // 2, init_kernel_size // 2)
        self.init_conv = nn.Conv3d(channels, init_dim, (1, init_kernel_size, 1), padding = init_padding)
    ...
    def forward(
        ...
        for block1, block2, spatial_attn, temporal_attn, upsample in self.ups:
            ...
            # Adjusting dimensions to match for concatenation with padding
            if x.shape[3] < h_tensor.shape[3]:
                pad_size = h_tensor.shape[3] - x.shape[3]
                x = F.pad(x, (0, 0, 0, pad_size, 0, 0, 0, 0))  # Pad the last dimension
            elif x.shape[3] > h_tensor.shape[3]:
                pad_size = x.shape[3] - h_tensor.shape[3]
                h_tensor = F.pad(h_tensor, (0, 0, 0, pad_size, 0, 0, 0, 0))  # Pad the last dimension
                
            x = torch.cat((x, h_tensor), dim=1)
...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape, cond = None, cond_scale = 1.):
        ...
        return img # unnormalize_img(img)
    ...
    def sample(self, cond = None, cond_scale = 1., batch_size = 16):
        ...
        return self.p_sample_loop((batch_size, channels, num_frames, image_size, 1), cond = cond, cond_scale = cond_scale)
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if not (x.shape[3] == img_size or x.shape[4] == img_size):
            raise ValueError(f"Input height or width must be {img_size}")
...
def normalize_img(t): # If t is in the range [0, 1], then normalize_img(t) will be in the range [-1, 1].
