...
import sys
sys.path.insert(1, '.')
from diffusion_policy.common.trans_utils import convert_position_rotation_to_homogeneous
...
class RobotKinHelper:
    def __init__(self, robot_name, tool_name=None, N_per_link=100, N_tool=100, debug=False):
        assert robot_name == 'ur5e', 'Only ur5e is supported for now'
        ...
        self.mesh_cache = []
        self.link_pcds = []
        self.initialize_mesh_cache(N_per_link)
    ...
    def initialize_mesh_cache(self, N_per_link):
        """Cache meshes and precompute normals for each link."""
        for link_idx in range(self.robot_model.numLinks()):
    ...
    def get_robot_pcd(self, qpos, N_joints=None, N_per_link=200, surface_only=False, out_o3d=True):
        ...
        for link_idx in range(start_link_idx, self.robot_model.numLinks()):
