...
class BlockPushLowdimRunner(BaseLowdimRunner):
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for prefix, value in total_rewards.items():
            ...
            std_value = np.std(value)  # Calculate standard deviation
            log_data[f'{prefix}mean_score'] = mean_value
            log_data[f'{prefix}std_score'] = std_value  # Log standard deviation

        for prefix, value in total_p1.items():
            mean_value = np.mean(value)
            std_value = np.std(value)  # Calculate standard deviation
            log_data[f'{prefix}p1'] = mean_value
            log_data[f'{prefix}std_p1'] = std_value  # Log standard deviation

        for prefix, value in total_p2.items():
            mean_value = np.mean(value)
            std_value = np.std(value)  # Calculate standard deviation
            log_data[f'{prefix}p2'] = mean_value
            log_data[f'{prefix}std_p2'] = std_value  # Log standard deviation
