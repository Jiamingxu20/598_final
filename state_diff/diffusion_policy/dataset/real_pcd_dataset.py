...
from diffusion_policy.real_world.real_data_conversion import (real_data_to_replay_buffer,
    real_zarr_to_replay_buffer, real_hdf5_to_replay_buffer)
...
from diffusion_policy.common.cv2_util import visualize_6d_trajectory
import cProfile
import pstats
import pathlib
...
class RealPcdDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store, rotation_transformer, save_format='hdf5', debug=False):
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            spatial_keys.append(key)
            c, n = shape
            out_resolutions[key] = (c, n)
            
    action_shape = tuple(shape_meta['action']['shape'])
    # assert action_shape in [(2,),(6,),(7,), (9,), (10,), (20,)]

    # load data
    cv2.setNumThreads(1)
    with threadpool_limits(1):
        if save_format == 'hdf5':
            replay_buffer = real_hdf5_to_replay_buffer(
                dataset_path=dataset_path,
                out_store=store,
                shape_meta=shape_meta,
                rotation_transformer=rotation_transformer,
