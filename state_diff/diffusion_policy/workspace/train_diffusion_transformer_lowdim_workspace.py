...
from diffusion_policy.policy.diffusion_traj_transformer_lowdim_policy import DiffusionTrajTransformerLowdimPolicy
...
class TrainDiffusionTransformerLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if 'traj' in cfg.policy._target_:
            self.model: DiffusionTrajTransformerLowdimPolicy
            self.ema_model: DiffusionTrajTransformerLowdimPolicy = None
        else:
    ...
    def run(self):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if not torch.is_tensor(raw_loss_dict):
                            train_diffuse_losses.append(raw_loss_dict['diffuse_loss'].item())
                            train_inv_losses.append(raw_loss_dict['inv_loss'].item())
                        step_log = {
                ...
                if len(train_diffuse_losses) > 0:
                    step_log['train_diffuse_loss'] = np.mean(train_diffuse_losses)
                if len(train_inv_losses) > 0:
                    step_log['train_inv_loss'] = np.mean(train_inv_losses)
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        if len(val_diffuse_losses) > 0:
                            val_diffuse_loss = torch.mean(torch.tensor(val_diffuse_losses)).item()
                        ...
                        # Calculate and log average validation losses
                        if len(val_losses) > 0:
