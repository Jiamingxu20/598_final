...
from diffusion_policy.policy.diffusion_traj_unet_hybrid_image_policy import DiffusionTrajUnetHybridImagePolicy
...
class TrainDiffusionUnetHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if 'traj' in cfg.policy._target_:
            self.model: DiffusionTrajUnetHybridImagePolicy = hydra.utils.instantiate(cfg.policy)
            self.ema_model: DiffusionTrajUnetHybridImagePolicy = None
        else:
    ...
    def run(self, rank=0, world_size=1):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                step_log['stage'] = int(underlying_model.training_stage) 
                        ...
                        if len(val_losses) > 0:
                            ...
                            if 'traj' in cfg.policy._target_:
                                if epochs_without_improvement >= epochs_waited_before_next_stage:
                                    ...
                                    #     underlying_model.set_training_mode('train_all')
                                    #     epochs_without_improvement = 0
                                    #     best_val_loss = float('inf')

                                    # elif underlying_model.training_stage == 2:
                                    #     print('State prediction training stage 2 completed. Moving to stage 3')
                                    #     underlying_model.set_training_mode('train_all')
                                    #     epochs_without_improvement = 0
                                    #     best_val_loss = float('inf')
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0 and (rank == 0):
                    ...
                    metric_dict['stage'] = underlying_model.training_stage
